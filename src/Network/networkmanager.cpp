...
NetworkManager::NetworkManager(QObject *parent)
    : QObject(parent), m_networkManager(new QNetworkAccessManager(this)),
      m_baseUrl("http://120.48.95.51:7001"), m_timeout(30000) // 默认30秒超时
      ,
...
QNetworkRequest NetworkManager::createRequest(const QString &url) {
  ...
  PersistentCookieJar *cookieJar =
      qobject_cast<PersistentCookieJar *>(m_networkManager->cookieJar());
  if (cookieJar) {
...
void NetworkManager::getWithRetry(const QString &url,
  ...
  QString queryData =
      queryParams.isEmpty() ? "(无查询参数)" : queryParams.toString();
  ...
  connect(reply, &QNetworkReply::finished, this,
          [this, reply, successCallback, errorCallback, url, queryParams,
           retryCount]() {
            // 检查是否是超时错误
            if (reply->error() == QNetworkReply::OperationCanceledError) {
              qWarning() << "NetworkManager - Request timed out for URL:"
                         << url;

              // 如果还有重试次数，则重试
              if (retryCount < m_maxRetries) {
                qWarning() << "NetworkManager - Retrying... ("
                           << (retryCount + 1) << "/" << m_maxRetries << ")";
                reply->deleteLater();

                // 延迟重试（指数退避）
                QTimer::singleShot(1000 * (retryCount + 1), this,
                                   [this, url, successCallback, errorCallback,
                                    queryParams, retryCount]() {
                                     getWithRetry(url, successCallback,
                                                  errorCallback, queryParams,
                                                  retryCount + 1);
                                   });
                return;
              } else {
                ...
                if (errorCallback) {
                  errorCallback(QString("Request timed out after %1 retries")
                                    .arg(m_maxRetries));
                }
                reply->deleteLater();
                return;
...
void NetworkManager::setBaseUrl(const QString &baseUrl) { m_baseUrl = baseUrl; }
...
void NetworkManager::post(const QString &url, const QJsonObject &data,
  ...
  // LOG_DEBUG() << "请求URL:" << fullUrl << "请求的数据为:" <<
  ...
  connect(reply, &QNetworkReply::finished, this,
          [this, reply, successCallback, errorCallback]() {
            handleResponse(reply, successCallback, errorCallback);
          });
}

void NetworkManager::put(const QString &url, const QJsonObject &data,
                         const SuccessCallback &successCallback,
                         const ErrorCallback &errorCallback) {
  QString fullUrl = buildFullUrl(url);
  QNetworkRequest request = createRequest(fullUrl);
...
void NetworkManager::handleResponse(QNetworkReply *reply,
  ...
  if (reply->error() == QNetworkReply::NoError) {
    ...
    LOG_DEBUG() << "请求URL:" << requestUrl
                << "请求响应数据:" << doc.toJson(QJsonDocument::Compact);
    if (doc.isObject()) {
    ...
    qWarning() << "NetworkManager - Error:" << errorMsg
               << "HTTP Status:" << statusCode;

    // 读取并显示服务器的完整响应
    QByteArray responseData = reply->readAll();
    if (!responseData.isEmpty()) {
      qWarning() << "NetworkManager - Response Data:"
...
void NetworkManager::uploadFile(const QString &url, const QString &filePath,
  ...
  filePart.setHeader(QNetworkRequest::ContentDispositionHeader,
                     QVariant("form-data; name=\"" + fileFieldName +
                              "\"; filename=\"" +
