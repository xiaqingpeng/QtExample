...
# 5.1 获取分析趋势
echo "5.1 测试分析趋势接口..."
TRENDS_RESPONSE=$(curl -s "$BASE_URL/api/analytics/trends?startDate=$START_DATE&endDate=$END_DATE" \
  -H "Authorization: Bearer $TOKEN")
...
# 5.2 获取页面访问统计
echo -e "\n5.2 测试页面访问统计接口..."
PAGE_VIEWS=$(curl -s "$BASE_URL/api/analytics/page-views?startDate=$START_DATE&endDate=$END_DATE" \
  -H "Authorization: Bearer $TOKEN")
...
# 5.3 获取事件统计
echo -e "\n5.3 测试事件统计接口..."
EVENT_STATS=$(curl -s "$BASE_URL/api/analytics/event-stats?startDate=$START_DATE&endDate=$END_DATE" \
  -H "Authorization: Bearer $TOKEN")
...
if echo "$SINGLE_EVENT" | jq -e '.code' >/dev/null 2>&1 && [ "$(echo "$SINGLE_EVENT" | jq -r '.code')" = "0" ]; then
...
if echo "$BATCH_EVENTS" | jq -e '.code' >/dev/null 2>&1 && [ "$(echo "$BATCH_EVENTS" | jq -r '.code')" = "0" ]; then
...
if echo "$BATCH_UPDATE" | jq -e '.code' >/dev/null 2>&1; then
...
if echo "$INVALID_TOKEN" | jq -e '.code' >/dev/null 2>&1 && [ "$(echo "$INVALID_TOKEN" | jq -r '.code')" != "0" ]; then
...
if echo "$MISSING_PARAMS" | jq -e '.success' >/dev/null 2>&1 && [ "$(echo "$MISSING_PARAMS" | jq -r '.success')" = "false" ]; then
    ...
    test_result "缺少必填参数验证" "FAIL" "$MISSING_PARAMS"
fi
